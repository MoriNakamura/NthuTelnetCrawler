...
def data_cleaner2():
    ...
    cur = sqlite.cursor()
    cur2 = sqlite.cursor()
    for row in cur.execute('SELECT id, content FROM articles;'):
        content = row[1]
        new_content = ''
        while content:
            nl = content.find('\n')
            if nl == -1:
                ...
                continue
            line = content[0:nl+1]
            if not line.startswith(">") and not line.startswith("※ 引述"):
                new_content += line
            content = content[content.find('\n')+1:]

        cur2.execute("UPDATE articles set content = :what WHERE id = :which;", {"what": new_content, "which":row[0]})
        if row[0] %50 == 0:
            print(row[0])
            sqlite.commit()
    sqlite.commit()
